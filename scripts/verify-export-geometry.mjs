...
const buildResult = await build({
  ...
  build: {
    write: false,
    target: "esnext",
    lib: { entry: resolve("scripts/export-geometry-check-entry.ts"), formats: ["es"] },
...
console.log(
  `Export geometry verified: ${result.width.toFixed(2)} × ${result.height.toFixed(2)} × ${result.depth.toFixed(2)} mm, ${result.vertices} vertices, ${result.stlBytes} STL bytes, ${result.parts} separate parts, ${result.zCases} Z-placement cases, ${result.detachedCases} detached-initial cases, ${result.photoWindowCases} photo-window modes.`,
);
